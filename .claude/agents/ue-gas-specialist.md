...
description: "[DEPRECATED — 본 프로젝트(ProjectFreeHero)에서 비활성] The Gameplay Ability System specialist owns all GAS implementation. 이 프로젝트는 BP-only 정책이며 GAS의 UAttributeSet이 C++ 상속을 요구하므로 사용하지 않습니다. 어빌리티/어트리뷰트/이펙트 관련 작업은 ue-blueprint-specialist로 라우팅됩니다."
...
> ## ⛔ 본 프로젝트(ProjectFreeHero)에서 이 에이전트는 비활성입니다
>
> **비활성 사유:** ProjectFreeHero는 **BP-only 정책**을 채택했습니다.
> GAS의 `UAttributeSet`은 C++ 상속을 요구하며, UBG(Ultimate Engine CoPilot) 플러그인 역시
> C++ 워크플로를 주 타겟으로 하지 않습니다. 따라서 GAS 전체 스택(Ability, AttributeSet,
> GameplayEffect, GameplayCue, GameplayAbilityTask)을 본 프로젝트에서는 사용하지 않습니다.
>
> **라우팅 규칙:** 어빌리티 / 어트리뷰트 / 이펙트 / 큐 관련 작업은 모두
> **`ue-blueprint-specialist`** 로 라우팅합니다. 해당 에이전트가 BP 기반 대체 구현
> (`UAttributeComponent`, `UAbilityComponent`, `UAbilityDataAsset`, `UEffectDataAsset`,
> Niagara + BP 이벤트 기반 Cue) 을 담당합니다.
>
> **GameplayTag는 유지:** `FGameplayTag` / `FGameplayTagContainer` 는 BP 완전 지원이므로
> 태그 시스템만은 그대로 사용합니다. 태그 관련 질문은 `ue-blueprint-specialist` 가 처리합니다.
>
> **이 파일은 참고용으로만 보존**되며, 본 프로젝트에서 스폰되지 않아야 합니다.
> 다른 프로젝트(C++/GAS 허용)에서 이 에이전트 정의를 재사용할 수 있도록 원본 내용은 아래에 유지합니다.
