...
**현 단계(PoC · 프로토타이핑) 기본값은 바이브코딩 모드입니다.** `.claude/skills/vibe/SKILL.md` 규칙 적용:

- CCGS 5단계 의식(질문→선택지→결정→초안→승인) 생략, 바로 코드
- 서브에이전트 자동 스폰 금지 (사용자 명시 요청 시만)
- 무거운 스킬 체인(`/gdd-*`, `/review-*`, `/consistency-check`, `/gate-check`) 자동 실행 금지
- 파일 작성 후 자동 컴파일 검증
- 3옵션 분기형 질문 생략 — 최선의 하나 골라서 실행, 틀리면 갈아엎을 준비

**의식 모드(아래 "협업 프로토콜")로 자동 전환되는 신호**:

- 팀원 합류 / 플레이테스트 예정 / 상업 출시 결정
- `.as` 파일 10개 초과 / 한 파일 150줄 초과
- 같은 버그 2회 이상 재발

사용자가 "의식 모드로"라고 명시 요청하면 해당 턴만 전환. 전체 가이드는 `docs/vibe-coding-guide.md` 참조.

## 협업 프로토콜 (의식 모드 — 기본 아님)

> 아래는 위 "기본 작업 모드" 섹션의 자동 전환 신호가 뜨거나 사용자가 명시 요청할 때만 적용됩니다.
